...
Item thumbnails (`data/images/`) are 64×64 PNGs sourced from Serebii's item icons, extracted from saved Safari web archives of Serebii's items page (not scraped live — this environment can't reach serebii.net). 609 of 726 items are covered so far; the Matcher UI just omits the thumbnail for anything without a matching file (matched by item ID, `data/images/<id>.png`). Serebii's live items catalog has substantially more entries than this dataset's 726 (terrain/wallpaper variants, seeds, saplings, and similar pieces outside the 43 tracked preference categories) — extracted images that don't match a current item by normalized name are simply not used. To extend coverage further, save a fresh web archive of Serebii's Pokopia items page(s) and re-run the same extraction (unzip → read the embedded image resources → match by normalized item name → resize to 64×64).
