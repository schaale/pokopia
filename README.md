...
Item thumbnails (`data/images/<item id>.png`) are 64×64 PNGs sourced from Serebii's item icons, extracted from saved Safari web archives of Serebii's items page (not scraped live — this environment can't reach serebii.net). All 726 items are covered. Extraction matches Serebii's image filename convention: lowercase the item name, strip accents (Serebii's URLs drop them, e.g. "Café sign" → `cafesign.png`), and remove spaces.
